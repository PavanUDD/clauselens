...
VENDOR_RULES: list[Rule] = [
    ...
    Rule(
        rule_id="VENDOR_R016",
        name="Unlimited vendor liability — no cap",
        contract_types=["vendor"],
        severity="HIGH",
        category="legal",
        detection_patterns=[
            r"liability.{0,60}shall\s+not\s+be\s+limited",
            r"no\s+limitation.{0,40}liability",
            r"unlimited\s+liability",
            r"liability.{0,40}not\s+limited",
            r"shall\s+have\s+no\s+obligation\s+to\s+carry\s+insurance",
        ],
        semantic_queries=["unlimited liability no cap on damages"],
        plain_english="There is no cap on liability — you could face unlimited financial exposure.",
        explanation_template="This contract contains no limitation of liability clause. Standard MSAs cap liability at total fees paid. Unlimited liability is a critical red flag.",
        typical_range="Liability capped at total fees paid under the agreement",
        negotiation_script="Can we add a mutual liability cap equal to the total fees paid in the prior 12 months? Unlimited liability is not standard.",
    ),

    Rule(
        rule_id="VENDOR_R017",
        name="One-sided indemnification including vendor negligence",
        contract_types=["vendor"],
        severity="HIGH",
        category="legal",
        detection_patterns=[
            r"indemnif.{0,60}regardless\s+of\s+cause",
            r"indemnif.{0,60}including.{0,40}negligence",
            r"arising\s+from\s+vendor'?s?\s+negligence",
            r"client\s+shall\s+indemnif.{0,60}vendor",
            r"hold\s+harmless.{0,60}regardless\s+of",
        ],
        semantic_queries=["indemnify vendor including their own negligence"],
        plain_english="You must indemnify the other party even for their own negligence — this is extremely one-sided.",
        explanation_template="This indemnification clause requires you to cover costs arising from the vendor's own negligence. This is a serious red flag that goes far beyond standard mutual indemnification.",
        typical_range="Mutual indemnification limited to each party's own negligence",
        negotiation_script="Can we limit indemnification to each party's own gross negligence or willful misconduct? Indemnifying the other party for their own negligence is not standard.",
    ),

    Rule(
        rule_id="VENDOR_R018",
        name="Vendor owns all work product and IP",
        contract_types=["vendor"],
        severity="HIGH",
        category="ip",
        detection_patterns=[
            r"work\s+product.{0,60}owned\s+(exclusively\s+)?by\s+vendor",
            r"vendor.{0,40}own.{0,40}(exclusively|all).{0,40}(work\s+product|deliverable|code|ip)",
            r"all.{0,40}(deliverable|work\s+product|code).{0,60}(owned|belong).{0,40}vendor",
            r"client\s+is\s+granted\s+a\s+limited.{0,60}license",
            r"nonexclusive.{0,40}license.{0,40}(use|internal)",
        ],
        semantic_queries=["vendor owns all deliverables client gets license only"],
        plain_english="The vendor owns everything they build for you — you only get a limited license to use it.",
        explanation_template="This contract gives the vendor ownership of all work product, code, and deliverables. You receive only a limited license. If the contract ends, you may lose access to everything built for you.",
        typical_range="Client owns deliverables paid for. Vendor retains pre-existing IP with license grant.",
        negotiation_script="Can we change this so client owns all deliverables created specifically for this engagement? Vendor can retain pre-existing tools and frameworks with a license grant.",
    ),

    Rule(
        rule_id="VENDOR_R019",
        name="Asymmetric termination — vendor can exit freely, client cannot",
        contract_types=["vendor"],
        severity="HIGH",
        category="termination",
        detection_patterns=[
            r"vendor\s+may\s+terminat.{0,60}any\s+(time|reason)",
            r"terminat.{0,40}for\s+any\s+reason\s+or\s+no\s+reason",
            r"client\s+may\s+terminat.{0,60}only\s+for",
            r"client\s+may\s+only\s+terminat",
            r"(90|sixty|ninety)\s+days.{0,40}(cure|breach).{0,40}client",
        ],
        semantic_queries=["vendor can terminate anytime client can only terminate for breach"],
        plain_english="The vendor can cancel anytime for any reason but you can only cancel after 90 days notice and proving a breach.",
        explanation_template="Termination rights are completely one-sided. The vendor can exit with 30 days notice for any reason while you must give 90 days notice and prove a material breach. This is highly unfavorable.",
        typical_range="Equal termination rights for both parties with same notice period",
        negotiation_script="Can we make termination rights equal? Both parties should have the same notice period and same right to terminate for convenience.",
    ),

    Rule(
        rule_id="VENDOR_R020",
        name="Fees non-refundable and due through end of term on termination",
        contract_types=["vendor"],
        severity="HIGH",
        category="payment",
        detection_patterns=[
            r"fees.{0,40}non.?refundable",
            r"all\s+fees\s+paid.{0,40}non.?refundable",
            r"liable\s+for\s+all\s+fees.{0,60}(remainder|remaining|rest).{0,40}term",
            r"fees\s+due\s+through.{0,40}(term|period|renewal)",
            r"remains?\s+liable\s+for\s+all\s+fees",
        ],
        semantic_queries=["all fees non-refundable and owe fees for rest of contract term"],
        plain_english="You cannot get any money back and you still owe all fees through the end of the contract term even if you terminate.",
        explanation_template="This clause requires payment of all fees through the end of the term regardless of termination. Combined with the vendor's right to terminate anytime, this creates extreme financial risk.",
        typical_range="Pro-rata refund for unused services on termination",
        negotiation_script="Can we add a pro-rata refund provision and limit liability on termination to services actually delivered?",
    ),

    Rule(
        rule_id="VENDOR_R021",
        name="Client non-compete — cannot hire vendor staff or use competitors",
        contract_types=["vendor"],
        severity="HIGH",
        category="restriction",
        detection_patterns=[
            r"client\s+shall\s+not.{0,60}(hire|engage|solicit).{0,40}(employee|contractor|subcontractor)",
            r"client.{0,40}not.{0,60}engage.{0,40}competing\s+vendor",
            r"substantially\s+similar\s+services",
            r"non.?solicit.{0,60}client\s+shall\s+not",
            r"24.{0,10}month.{0,40}(hire|engage|solicit|compet)",
        ],
        semantic_queries=["client cannot hire vendor employees or use competitors for 24 months"],
        plain_english="For 24 months after this contract you cannot hire the vendor's staff or use any competitor for similar services.",
        explanation_template="This is an extremely broad restriction on the client. A 24-month ban on using competing vendors is unusual and limits your business flexibility significantly. This clause effectively locks you into the vendor.",
        typical_range="12-month non-solicitation of named employees only. No restriction on using competitors.",
        negotiation_script="Can we remove the competing vendor restriction entirely and narrow the non-solicitation to direct solicitation of specifically named individuals for 12 months maximum?",
    ),
]
